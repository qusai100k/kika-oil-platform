...
async function main(){
  ...
  const sectionKeys=["basic","characteristics","safety","screening","routine","actives","goals","preferences"];
  const sections=new Map<string,string>();
  for(let i=0;i<sectionKeys.length;i++){const key=sectionKeys[i];const row=await prisma.assessmentSection.upsert({where:{templateId_key:{templateId:template.id,key}},update:{sortOrder:i},create:{templateId:template.id,key,titleAr:key,sortOrder:i}});sections.set(key,row.id)}
  const questions=[
  ...
  await prisma.assessmentQuestion.updateMany({where:{code:{in:dependent.map(x=>x[0])},version:1},data:{templateId:template.id,sectionId:sections.get("safety")}});
}
